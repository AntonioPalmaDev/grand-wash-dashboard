...
export default function Dashboard() {
  ...
  const chartData = useMemo(() => {
    ...
    completed.forEach(op => {
      ...
      byMonth[key] = (byMonth[key] || 0) + op.valorBruto;
    });
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="glass-card rounded-lg p-5">
          <h2 className="text-lg font-semibold mb-4">Movimentação por Mês</h2>
          {chartData.length === 0 ? (
            <p className="text-muted-foreground text-sm">Nenhuma operação concluída ainda.</p>
          ) : (
            <ResponsiveContainer width="100%" height={280}>
              <BarChart data={chartData}>
                <CartesianGrid strokeDasharray="3 3" stroke="hsl(260,12%,18%)" />
                <XAxis dataKey="month" stroke="hsl(260,8%,55%)" fontSize={12} />
                <YAxis stroke="hsl(260,8%,55%)" fontSize={12} tickFormatter={v => `R$${(v/1000).toFixed(0)}k`} />
                <Tooltip
                  contentStyle={{ background: "hsl(260,18%,10%)", border: "1px solid hsl(260,12%,18%)", borderRadius: 8 }}
            ...
            <div className="space-y-3">
              {topClients.map((c, i) => (
                <div key={i} className="flex items-center justify-between p-3 rounded-md bg-secondary/30">
                  ...
                  <span className="font-mono text-sm">{formatCurrency(c.total)}</span>
                </div>
              ))}
            </div>
          )}
